...
export class AttendanceService {
    ...
    async getAll(userId: number) {
        ...
        return records
            .filter((record) => this.normalizeName(record.Name) === this.normalizeName(currentUser.name))
    ...
    async getAvailability(userIds?: number[]) {
        ...
        return users.map((user) => {
            ...
            return {
                ...
                attendanceId: record ? `${this.cleanDateString(record.DateString)}|${this.normalizeEmpcode(record.Empcode)}` : null,
    ...
    private async fetchExternalAttendance(): Promise<ExternalAttendanceItem[]> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`[Attendance API] Status ${response.status}: ${errorText}`);
            ...
            if (json?.Error !== false || !Array.isArray(json?.InOutPunchData)) {
                ...
                throw new InternalServerErrorException('Attendance API returned an invalid payload');
            }
            ...
            throw new InternalServerErrorException(`Failed to fetch attendance data: ${error.message}`);
        }
    }
    ...
    private getAvailabilityStatus(
        ...
        if (record.Status === 'A' || record.Status === 'WO' || normalized.checkIn === '00:00') {
            return 'ABSENT';
        }
