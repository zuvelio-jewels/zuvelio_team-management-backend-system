  ...
  private async sendSetupPackage(
    ...
    if (installerPath) {
      ...
      const fallbackInstaller = [
        ...
        '',
        ':: ── Elevation via VBS (most reliable — keeps THIS window open) ─────────',
        'net session >nul 2>&1',
        'if %errorlevel% neq 0 (',
        '    echo Requesting administrator access...',
        '    echo Please click YES on the Windows security prompt.',
        '    echo.',
        '    :: Write a tiny VBS that re-runs this exact script as admin',
        '    set "VBS=%TEMP%\\zuvelio_uac_%RANDOM%.vbs"',
        '    echo Set o=CreateObject("Shell.Application") > "!VBS!"',
        '    echo o.ShellExecute "%~f0", "", "%~dp0", "runas", 1 >> "!VBS!"',
        '    wscript //nologo "!VBS!"',
        '    del "!VBS!" >nul 2>&1',
        '    exit /b',
        ')',
        ...
        '',
        '    if !DL_OK!==0 (',
        '        powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $wc=New-Object System.Net.WebClient; $wc.Headers.Add(\'User-Agent\',\'Mozilla/5.0\'); $wc.DownloadFile(\'%DL_URL%\',\'%~dp0zuvelio-activity-agent.exe\'); Write-Host \'Download OK.\' } catch { Write-Host (\'FAIL: \'+$_.Exception.Message) }"',
        '        if !errorlevel! equ 0 set DL_OK=1',
        '    )',
        '',
        '    if !DL_OK!==0 (',
        ...
        '        echo Rename the file to:  zuvelio-activity-agent.exe',
        '        echo Put it in this folder, then double-click EMPLOYEE_SETUP.bat again.',
        '        echo.',
        '        pause',
        '        exit /b 1',
        '    )',
        ...
        '    echo Please download a fresh setup package from the Zuvelio portal.',
        '    echo.',
        '    pause',
        '    exit /b 1',
        ')',
        '',
        'echo Stopping any existing agent instance...',
        ...
        '',
        'echo Starting activity agent now...',
        'start "" /B "%INSTALL_DIR%\\zuvelio-activity-agent.exe"',
        '',
        ':: Retry check up to 5 times (2s each = 10s total)',
        'set TRIES=0',
        ':CHECKLOOP',
        'timeout /t 2 /nobreak >nul',
        ...
        '',
        'echo.',
        'echo Agent launched. Tracking will begin within 1 minute.',
        'echo Run CHECK_STATUS.bat to verify at any time.',
        'echo.',
        'pause',
        '',
      ].join('\r\n');
