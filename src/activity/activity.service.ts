import { Injectable, Logger, ForbiddenException, UnauthorizedException, NotFoundException } from '@nestjs/common';
...
export class ActivityService {
    ...
    async logActivityBatch(userId: number, activityEvents: CreateActivityEventDto[]) {
        try {
            ...
            const canAcceptEvent = await this.canAcceptActivityEvent(userId);
            if (!canAcceptEvent) {
                return { success: false, reason: 'Monitoring not active', acceptedEvents: 0 };
            }
    ...
    private isWithinWorkingHours(startWorkHour: number, endWorkHour: number): boolean {
     ...
     */
    private isWithinWorkingHoursInclusive(startWorkHour: number, endWorkHour: number): boolean {
        const currentHour = new Date().getHours();
        return currentHour >= startWorkHour && currentHour <= endWorkHour;
    }
    ...
    async isMonitoringActive(userId: number): Promise<boolean> {
        try {
            ...
            if (!this.isWithinWorkingHoursInclusive(config.startWorkHour, config.endWorkHour)) {
                return false;
            }

            return true;
        } catch (error) {
            this.logger.error(`Error checking monitoring status for user ${userId}:`, error);
            return false;
        }
    }
    ...
    async getActivitySummary(
        ...
        if (userId !== currentUserId) {
            throw new ForbiddenException('Cannot access activity data of other users');
        }
        ...
        try {
            ...
            this.logger.error(`Error fetching activity summary for user ${userId}:`, error);
            throw error;
        }
    }
    ...
    async getTeamActivity(managerId: number, query: GetActivitySummaryDto) {
        try {
            ...
            if (!manager || (manager.role !== 'MANAGER' && manager.role !== 'ADMIN')) {
                throw new ForbiddenException('Only managers and admins can access team activity');
            }
            ...
            this.logger.error(`Error fetching team activity for manager ${managerId}:`, error);
            throw error;
        }
    }
    ...
    async getTodayActivityDashboard(userId: number) {
        try {
            ...
            return {
                ...
                totalKeystrokes: todaySummaries.reduce((sum, s) => sum + s.totalKeystrokes, 0),
                totalClicks: todaySummaries.reduce((sum, s) => sum + s.totalClicks, 0),
                totalIdleMinutes: todaySummaries.reduce((sum, s) => sum + s.idleTimeMinutes, 0),
                monitoringActive,
                workingHours: config
    ...
    async getMonitoringConfig(userId: number) {
        try {
            ...
            this.logger.error(`Error fetching monitoring config for user ${userId}:`, error);
            throw error;
        }
    }
    ...
    async updateMonitoringConfig(
        ...
        try {
            ...
            if (currentUserRole !== 'ADMIN') {
                throw new ForbiddenException('Only admins can update monitoring configuration');
            }
    ...
    async aggregateHourlyStats() {
        try {
            ...
            for (const [_, group] of groupedByUserTask) {
                ...
                const clicks = group.events.filter((e) => e.eventType === 'CLICK').length;
    ...
    async authenticateWithDeviceToken(token: string) {
        ...
        const payload = { id: user.id, email: user.email, name: user.name, role: user.role, isActive: user.isActive };
    ...
    async revokeDeviceToken(adminRole: string, tokenId: number) {
        if (!['ADMIN', 'MANAGER'].includes(adminRole)) {
            throw new ForbiddenException('Only admins and managers can revoke device tokens');
        }
        const device = await this.prisma.deviceToken.findUnique({ where: { id: tokenId } });
        if (!device) throw new NotFoundException('Device token not found');

        await this.prisma.deviceToken.update({ where: { id: tokenId }, data: { isRevoked: true } });
        return { success: true, message: 'Device token revoked' };
    }

    async listDeviceTokens(adminRole: string, userId?: number) {
        if (!['ADMIN', 'MANAGER'].includes(adminRole)) {
            throw new ForbiddenException('Only admins and managers can list device tokens');
        }
    ...
    async getAdminEmployeeList(adminRole: string) {
        if (!['ADMIN', 'MANAGER'].includes(adminRole)) {
            throw new ForbiddenException('Only admins and managers can access employee list');
        }
        ...
        const users = await this.prisma.user.findMany({
            select: {
                ...
                lastActivityAt: true,
                monitoringConfig: {
                    select: { isMonitoringEnabled: true, startWorkHour: true, endWorkHour: true, idleThresholdMinutes: true },
                },
                deviceTokens: {
    ...
    async toggleEmployeeMonitoring(adminRole: string, targetUserId: number, enabled: boolean) {
        if (!['ADMIN', 'MANAGER'].includes(adminRole)) {
            throw new ForbiddenException('Only admins and managers can toggle monitoring');
        }
        ...
        return { success: true, userId: targetUserId, isMonitoringEnabled: enabled };
    }
}
