...
export class ReportsService {
    ...
    private computeAlert(task: {
        ...
        if (task.personStatus === 'IN_PROGRESS') return isOverdue ? 'UNDER_PROCESS_TIMEOUT' : 'UNDER_PROCESS_IN_TIME';
        if (task.personStatus === 'STUCK') return isOverdue ? 'STUCK_WITH_TIMEOUT' : 'STUCK_IN_TIME';
        return isOverdue ? 'NOT_START_YET_TIMEOUT' : 'NOT_START_YET_IN_TIME';
    }

    /** Report for a single user (self) */
    async getUserReport(userId: number, from?: string, to?: string): Promise<UserReportStats> {
        ...
        const user = tasks[0]?.assignedTo ?? await this.prisma.user.findUnique({
            where: { id: userId },
    ...
    async getAllUsersReport(from?: string, to?: string): Promise<UserReportStats[]> {
        const where: any = {};
        if (from || to) {
            where.createdAt = {};
            if (from) where.createdAt.gte = new Date(from);
    ...
    private computeStats(
        ...
        const inProgress = tasksWithAlert.filter((t) => t.personStatus === 'IN_PROGRESS').length;
        const notStarted = tasksWithAlert.filter((t) => t.personStatus === 'NOT_STARTED').length;
        const stuck = tasksWithAlert.filter((t) => t.personStatus === 'STUCK').length;
        const rework = tasksWithAlert.filter((t) => t.alert === 'RE_WORK').length;
        const completedOnTime = tasksWithAlert.filter((t) => t.alert === 'COMPLETE_IN_TIME').length;
        const completedLate = tasksWithAlert.filter((t) => t.alert === 'LATE_COMPLETE_WITH_TIMEOUT').length;
        const onTimeRate = done > 0 ? Math.round((completedOnTime / done) * 100) : 0;
