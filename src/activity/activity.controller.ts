  ...
  private async sendSetupPackage(
    ...
    if (installerPath) {
      ...
      const fallbackInstaller = [
        ...
        '      "try { Invoke-WebRequest -Uri \'https://github.com/zuvelio-jewels/zuvelio_team-management-backend-system/releases/download/v2.0.0/zuvelio-activity-agent-updated.exe\' -OutFile \'%~dp0zuvelio-activity-agent.exe\' -UseBasicParsing; Write-Host \'Download complete.\' } catch { Write-Host (\'Download failed: \' + $_.Exception.Message); exit 1 }"',
        '    if errorlevel 1 (',
        '        echo.',
        ...
        '        echo and place zuvelio-activity-agent.exe in this folder, then run this script again.',
        '        pause',
        '        exit /b 1',
        '    )',
        '    set EXE_FOUND=1',
