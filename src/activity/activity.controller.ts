  ...
  private async sendSetupPackage(
    ...
    if (installerPath) {
      ...
      const fallbackInstaller = [
        ...
        '    if !errorlevel! equ 0 (',
        '        curl.exe -L --fail --retry 5 --retry-delay 3 --retry-max-time 300 -A "Mozilla/5.0" -o "%~dp0zuvelio-activity-agent.exe" "!DL_URL!"',
        ...
        `        echo  Contact your administrator to get a fresh ZuvelioSetup.zip`,
        '        echo.',
        '        pause',
        '        exit /b 1',
        '    )',
        ...
        '        echo.',
        '        echo ERROR: Downloaded file is too small (!EXE_SIZE! bytes) - download was incomplete.',
        '        del /F /Q "%~dp0zuvelio-activity-agent.exe" >nul 2>&1',
        '        echo  Please try again or contact your administrator.',
        '        echo.',
        '        pause',
        '        exit /b 1',
        '    )',
        ...
        '',
        'echo Starting agent...',
        'start "Zuvelio Agent" /D "!INSTALL_DIR!" /MIN "!INSTALL_DIR!\\zuvelio-activity-agent.exe"',
        '',
        'set TRIES=0',
        ':CHECKLOOP',
        'timeout /t 2 /nobreak >nul',
    ...
    const startAgentBat = [
      ...
      'start "Zuvelio Agent" /D "%CD%" /MIN zuvelio-activity-agent.exe',
      '',
    ...
    if (employeeSetupPath) {
      ...
      const employeeSetupFallback = [
        ...
        'echo.',
        ':: Use VBScript for reliable UAC elevation (handles paths with spaces/parentheses)',
        'set "ZUV_VBS=%TEMP%\\zuv_%RANDOM%.vbs"',
        '(',
        '  echo Set sh = CreateObject^("Shell.Application"^)',
        '  echo sh.ShellExecute "cmd.exe", "/K cd /D ""%~dp0"" ^&^& ""%~f0""", "", "runas", 1',
        ') > "%ZUV_VBS%"',
        'cscript //nologo "%ZUV_VBS%"',
        'del "%ZUV_VBS%" 2>nul',
        'if %errorlevel% neq 0 (',
        '    echo.',
        '    echo  ERROR: Could not get administrator access.',
        '    echo  Please right-click EMPLOYEE_SETUP.bat and choose "Run as administrator".',
        '    echo.',
        '    pause',
        ')',
        'exit /b',
        ...
        'if %errorlevel% neq 0 (',
        '    echo.',
        '    echo  Installation encountered an error. Please try again or',
